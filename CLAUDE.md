...
- `src/lib/site.ts` — name, **Fresha `bookingUrl` / `giftCardUrl`**, contact, address, hours, socials, `url` (domain for SEO), gift-card amounts. Also exports `fullAddress` plus `mapsLink` / `mapsEmbedUrl`, so the footer link, the contact page's "Get directions", and the map are all built in one place and can't drift onto different pins.
  - **The map you click and the map you see are two different providers, on purpose.** `mapsLink` is the clinic's Apple Maps place link (`site.mapUrl`); `mapsEmbedUrl` is an OpenStreetMap export embed centred on `site.lat/lng`, taken from that same Apple pin. **Apple cannot be the iframe** — `maps.apple.com` sends `X-Frame-Options: SAMEORIGIN`, so a frame pointed at it renders an empty box from any other domain. Google's keyless `output=embed` frames fine but is widely blocked by privacy extensions (that's the failure this replaced), and its keyed Embed API needs an API key nobody has here. Don't "fix" the embed back to either without solving those.
...
- **Real address set (2026-08-13):** 13720 Victory Blvd, Van Nuys, CA 91401 — replacing the placeholder. `addressLine2` is deliberately empty until the owner supplies the suite number; `fullAddress` drops empty parts so nothing renders a stray comma in the meantime. Phone and email are still placeholders. Note `site.city` is now "Van Nuys", which also drives the masthead tag and the "a licensed medical spa in …" line — swap to a separate field if the brand copy should say "Los Angeles" while the address stays Van Nuys.
