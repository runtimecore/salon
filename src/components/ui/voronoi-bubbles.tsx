...
// ("Voronoi"). Zero dependencies: one WebGL canvas that fills its
// parent. Drop it behind your content with
// <div className="relative"><ShaderBackground className="absolute inset-0" />…
// Adapted from Paper Shaders: https://shaders.paper.design/voronoi
...
vec3 shade(vec2 uv, vec2 p, float t) {
  vec2 grid = p * (3.0 + u_intensity * 9.0);
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      vec2 rnd = hash22(cell + offset + u_seed);
      vec2 point = offset + 0.5 + 0.42 * sin(t * 0.3 + 6.2831853 * rnd) - local;
  ...
  float edge = smoothstep(0.0, mix(0.18, 0.015, u_paramA), sqrt(second) - sqrt(nearest));
  return mix(palette(colourIndex), u_colors[0], edge * 0.7);
}
...
const UNIFORMS = {
  colors: [[0.403921568627451,0.9098039215686274,0.9764705882352941],[0.054901960784313725,0.4549019607843137,0.5647058823529412],[0.023529411764705882,0.06274509803921569,0.0784313725490196],[0.9254901960784314,0.996078431372549,1],[0.9254901960784314,0.996078431372549,1],[0.9254901960784314,0.996078431372549,1],[0.9254901960784314,0.996078431372549,1],[0.9254901960784314,0.996078431372549,1]] as [number, number, number][],
  colorCount: 4,
  scale: 1.920,
  intensity: 0.860,
  paramA: 0.320,
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    // Local change to the generated file: `render` below is a hoisted function
    // declaration, and TypeScript discards outer-scope narrowing inside those
    // (a declaration could in principle be called before the guard runs), so
    // `canvas.width` failed strict null checks. Re-binding with an explicit
    // non-null type makes the *declared* type correct, which needs no
    // assertions and leaves the rest of the file untouched. Re-apply this if
    // the shader is ever regenerated.
    ...
    // Same narrowing caveat as `canvas` above — `render` reads `gl` too.
